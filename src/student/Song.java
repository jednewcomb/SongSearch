...
public class Song implements Comparable {
    ...
    /**
     *
     * @return String artist - artist of song
     */
    ...
    /**
     *
     * @return String lyrics - lyrics of song
     */
    ...
    /**
     *
     * @return String title - title of song
     */
     ...
     * artist's songs are together, but in alpha order.
     */
    @Override
    public int compareTo(Object that) {
        int comp = 0;
        //if theres nothing in object that, to heck with it
        if (that == null) {
            comp = 1;
            //if objects this and that are equal, they are the same
        } else if (this == that) {
            comp = 0;
        } else {
            //if they are not the same, compare the artist ignoring cases
            comp = this.artist.compareToIgnoreCase(((Song) that).artist);
            if (comp == 0) {
                //if the artist are the same, compare titles
                comp = this.getTitle().compareToIgnoreCase(((Song) that).getTitle());
            }
        }
    ...
    public static class CmpTitle extends CmpCnt implements Comparator<Song>{
        ...
        public int compare(Song s1, Song s2){
            cmpCnt++;
            return s1.title.compareToIgnoreCase(((Song)s2).title);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        CmpArtist check = new CmpArtist(); 
        System.out.println("comparator test: ");
        System.out.println(check.compare(s1, s2));
        System.out.println(check.compare(s2, s1));
        System.out.println(check.compare(s1, s3));
        System.out.println(check.compare(s3, s1));
        System.out.println(check.compare(s1, s1));
    }
